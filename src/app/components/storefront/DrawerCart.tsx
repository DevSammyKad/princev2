...
import { CheckOutButton, DeleteItemButton } from '../dashboard/SubmitButton';
  ...
  return (
    <>
      <span>
        ...
        <strong className="font-medium text-green-500"> {saveAmount} </strong>{' '}
        on this purchase.
      </span>
      <div className="flex flex-col justify-start">
        {cart?.items.map((item) => (
          <div key={item.id} className="flex justify-between my-5 border-b p-3">
            <Image
        ...
        <span className="divide-y-0"></span>
      </div>
    </>
  );
}
